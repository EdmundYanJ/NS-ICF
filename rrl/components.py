...
class weight_net(nn.Module):
    def __init__(self, in_size=128, out_size=96, lmbda=0.2):
    ...
    def forward(self, ui_vec):
        return self.weight_linear(ui_vec)
...
class LRLayer(nn.Module):
    """The LR layer is used to learn the probability of class labels."""
...
def extract_rules(prev_layer, skip_connect_layer, layer, pos_shift=0):
    ...
    for ri, row in enumerate(Wb):
        # if ri % 500 == 0:
        #     print('{} / {}'.format(ri, Wb.shape[0]))
        # print(layer.node_activation_cnt[ri + pos_shift] == layer.forward_tot, layer.node_activation_cnt[ri + pos_shift], layer.forward_tot)
                    ...
                    if bi not in bound:
                        ...
                        if (ci < c.shape[0] // 2 and layer.layer_type == 'conjunction') or \
                           (ci >= c.shape[0] // 2 and layer.layer_type == 'disjunction'):
                            ...
                            func = min
                        # func = max if (ci < c.shape[0] // 2) else min
                        bound[bi][1] = func(bound[bi][1], c[ci])
                        if bound[bi][1] == c[ci]:
                            del rule[(-1, bound[bi][0])]
                            rule[(-1, i)] = 1
                            bound[bi][0] = i
                # elif skip_connect_layer is None:
...
class UnionLayer(nn.Module):
    """The union layer is used to learn the rule sets."""
    ...
    def get_rules(self, prev_layer, skip_connect_layer):
        ...
        # print(set(con_dim2id.values()))
        # print(set(dis_dim2id.values()))
        ...
        for rl, op in zip(self.rule_list, ('&', '｜')):
            ...
            for rule in rl:
                ...
                for i, ri in enumerate(rule):
                    ...
                    name += op_str + var_str
                    # print(prev_rule_name[ri[0]][ri[1]], end=' {} '.format(op))
