...
data=pd.read_csv(DATA_DIR+'/implicit_ratings.csv',header=None)
item_num=data[1].max()
user_num=data[0].max()
embedding_dim=64
...
def get_data_loader_train(train_dataset, valid_dataset, world_size, rank, batch_size, div_place, pin_memory=False,
    ...
    for x, x_valid, f in zip(X_df, X_df_valid, f_df):
        db_enc = DBEncoder(f, discrete=False)
        # db_enc = DBEncoder(f_df, discrete=True)
        db_enc.fit(x, y_df)
        db_enc_list.append(copy.deepcopy(db_enc))
        #train_set
        X, Y = db_enc.transform(x, y_df, normalized=True, keep_stat=True)
        train_set = TensorDataset(torch.tensor(X.astype(np.float32)), torch.tensor(Y.astype(np.float32)))
        train_sampler = torch.utils.data.distributed.DistributedSampler(train_set, num_replicas=world_size, rank=rank,shuffle=False)
        train_loader_list.append(
            DataLoader(train_set, batch_size=batch_size, shuffle=False, pin_memory=pin_memory, sampler=train_sampler))

        #valid set
        X_valid, Y_valid = db_enc.transform(x_valid, y_df_valid, normalized=True, keep_stat=True)
        valid_set = TensorDataset(torch.tensor(X_valid.astype(np.float32)), torch.tensor(Y_valid.astype(np.float32)))
        valid_sampler = torch.utils.data.distributed.DistributedSampler(valid_set, num_replicas=world_size, rank=rank,shuffle=False)
    ...
    uid_sampler = torch.utils.data.distributed.DistributedSampler(uid, num_replicas=world_size, rank=rank,shuffle=False)
    uid=DataLoader(uid,batch_size=batch_size,shuffle=False, pin_memory=pin_memory, sampler=uid_sampler)
    iid_sampler = torch.utils.data.distributed.DistributedSampler(iid, num_replicas=world_size, rank=rank,shuffle=False)
    iid = DataLoader(iid, batch_size=batch_size, shuffle=False, pin_memory=pin_memory, sampler=iid_sampler)

    valid_uid_sampler = torch.utils.data.distributed.DistributedSampler(valid_uid, num_replicas=world_size, rank=rank,shuffle=False)
    valid_uid = DataLoader(valid_uid, batch_size=batch_size, shuffle=False, pin_memory=pin_memory, sampler=valid_uid_sampler)
    valid_iid_sampler = torch.utils.data.distributed.DistributedSampler(valid_iid, num_replicas=world_size, rank=rank,shuffle=False)
...
def get_data_loader_test(train_dataset,test_dataset, world_size, rank, batch_size, div_place, pin_memory=False,
    ...
    for x_test, f in zip(X_df_test, f_df_test):
        db_enc = DBEncoder(f, discrete=False)
        # db_enc = DBEncoder(f_df, discrete=True)
        db_enc.fit(x_test, y_df_test)
        db_enc_list.append(copy.deepcopy(db_enc))
        #test_set
        X_test, Y_test = db_enc.transform(x_test, y_df_test, normalized=True, keep_stat=True)
        test_set = TensorDataset(torch.tensor(X_test.astype(np.float32)), torch.tensor(Y_test.astype(np.float32)))
        test_sampler = torch.utils.data.distributed.DistributedSampler(test_set, num_replicas=world_size, rank=rank,shuffle=False)
...
def load_model(path, device_id,  topk_target, log_file=None, distributed=True):
    ...
    for key in list(stat_dict.keys()):
        ...
        if key[:7]=='top_net':
            stat_dict[key[:7]+key[14:]] = stat_dict.pop(key)
        elif key[5:14]=='embedding' or key[:5]=='final':
            pass
        else:
            stat_dict[key[:17]+key[24:]] = stat_dict.pop(key)
...
def test_model(args):
    ...
    valid_dataset = args.valid_dataset
    db_enc_list, train_loader_list, valid_loader_list, uid, iid, valid_uid, valid_iid = get_data_loader_train(
                                    train_dataset, valid_dataset, args.world_size, 0, args.batch_size,
                                        div_place=div_place, pin_memory=True, save_best=args.save_best)
    with open(args.rrl_file, 'w') as rrl_file:
        rrl.rule_print(db_enc_list, train_loader_list, file=rrl_file)
